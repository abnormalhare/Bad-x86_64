...
void ASM_83AND(RM *rm, s32 disp, u8 val) {
    ...
    if (rm->isPtr) {
        ...
        switch (rm->otype) {
            case R_Bit16: { STACK16(temp, reg + disp); prev.x = *temp; *temp &= sval; break; }
            case R_Bit32: { STACK32(temp, reg + disp); prev.e = *temp; *temp &= sval; break; }
            case R_Bit64: { STACK64(temp, reg + disp); prev.r = *temp; *temp &= sval; break; }
            default: break;
        }
